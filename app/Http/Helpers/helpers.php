...
if (!function_exists('send_email')) {
    ...
    function send_email($to, $name, $subject, $message)
    {
        ...
        if ($gnl->email_notification == 1) {
            ...
            $to_name = $gnl->title;
            $to_email = $to;
            $from_name = $gnl->sitename;
            $from_email = $from;

            $data = array('name' => $gnl->title, "body" => $message);
//            dd($data,$to_email,$from_name,$to );
            $x = Mail::send('mail', $data, function ($messages) use ($to_name, $to_email, $from_name, $from_email, $subject) {
                $messages->to($to_email, $to_name)->subject($subject);
                $messages->from($from_email, $from_name);
            });
...
if (!function_exists('send_email_lang')) {
    function send_email_lang($to, $name, $message, $template_code, $lang, $subject = '', $is_admin = false)
    {

        $email = \App\EmailLanguage::where('code', $template_code)->where('lang', $lang)->first();
//        dd($email,$to, $name, $message, $template_code, $lang, $subject , $is_admin );
        $template = $email->message;
        if ($is_admin) {
            $template .= '<div>{{confirm}}</div>';
        }
        $subject = empty($subject) ? $email->subject : $subject;
        foreach ($message as $key => $value) {
            $template = str_replace('{{' . $key . '}}', $value, $template);
        }
        send_email($to, $name, $subject, $template);
    }
}
...
function showBelowUser($id)
{
    ...
    foreach ($under_ref as $data) {
        ...
        echo "<li class=\"container warning\" >";
        echo '<p style="color: black">++++++' . $print[] = $data->id . '++++</p>';

        if ($cc > 0) {
            echo '<ul >';
            echo '<li class="container " >';
            echo '<p style="color: black ">++++++' . $print[] = showBelowUser($data->id) . '++++</p>';
            echo '</li>';
            echo '</ul>';
        }
        echo "</li>";
