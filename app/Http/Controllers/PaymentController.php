...
class PaymentController extends Controller
{
    ...
    public function depositConfirm()
    {
        ...
        if (is_null($data)) {
            return redirect()->route('home')->with('alert',__('deposit.Invalid Deposit Request'));
        }
        ...
        if ($data->gateway_id == 101) {
            ...
            $btc = round($btcamount, 16);
            $data = Deposit::where('trx', $track)->orderBy('id', 'DESC')->first();
//dd($data->btc_wallet , $data->id);
...
//                dd($mysite_root);
                ...
                $invoice_id = $track;
                $callback_url = "http://lacura.me/ipnbtc?invoice_id=" . $invoice_id . "&secret=" . $secret;


                $resp = @file_get_contents($blockchain_receive_root . "v2/receive?key=" . $my_api_key . "&callback=" . urlencode($callback_url) . "&xpub=" . $my_xpub);
                if (!$resp) {
                    return redirect()->route('home')->with('alert', __('deposit.BLOCKCHAIN API HAVING ISSUE. PLEASE TRY LATER'));
                }
            ...
            if ($btcdata->status != 'success') {
                return back()->with('alert', __('deposit.Failed to Process'));
            }
            ...
            if ($data->btc_amo == 0 || $data->btc_wallet == "") {
                ...
                if ($result['error'] == 'ok') {
                    ...
                    $data['image'] = $result['result']['qrcode_url'];
                    $data['detail'] = $result['result']['status_url'];
                    $data->update();
                } else {
                    return back()->with('alert',  __('deposit.Failed to Process'));
                }
            ...
            $wallet = $data['btc_wallet'];
            $bcoin = $data['btc_amo'];
            $status_url = $data['detail'];
            $pt = $method->name;


            $qrurl = $data['image'];
            return view('user.payment.coinpaybtc', compact('bcoin', 'wallet', 'qrurl', 'pt', 'status_url'));
            ...
            if ($data->btc_amo == 0 || $data->btc_wallet == "") {
                ...
                if ($result['error'] == 'ok') {
                    ...
                    return back()->with('alert', 'deposit.Failed to Process');
                }
            }
            $data = Deposit::where('trx', $track)->orderBy('id', 'DESC')->first();
            $wallet = $data['btc_wallet'];
            ...
            if ($order) {
                ...
                return redirect()->route('home')->with('alert', __('deposit.Failed to Process'));
                exit();
            }
    ...
    public function coinPaymentsValidateIpn(Request $request)
    {
        try {
            /** @var Ipn $ipn */
            ...
            if ($ipn->isApi()) {
                ...
                $gatewayData = Gateway::find(501);
                $track = $ipn->invoice_id;
                $secret = $ipn->secret;
                $address = $ipn->address;
                $value = $ipn->value;
                $confirmations = $ipn->status;
                $value_in_btc = $ipn->value / 100000000;

                $trx_hash = $ipn->transaction_hash;

                $data = Deposit::where('trx', $track)->orderBy('id', 'DESC')->first();


                if ($data->status == 0) {
                    if ($data->btc_amo == $value_in_btc && $data->btc_wallet == $address && $secret == "lacura.me" && $confirmations >= 100 || $confirmations == 2) {
                        //Update User Data
                        $this->userDataUpdate($data);
                    }
                }
        ...
        try {
            ...
            try {
                ...
                if ($charge['status'] == 'succeeded') {
                    ...
                    //Update User Data
                    $this->userDataUpdate($data);
                    return redirect()->route('home')->with('success', __('deposit.Deposit Successful'));
                }
            ...
            $body = $e->getJsonBody();
            $err = $body['error'];
            return redirect()->route('home')->with('alert', $err['message']);

        } catch (\Stripe\Error\InvalidRequest $e) {
            return redirect()->route('home')->with('alert',  __("deposit.Invalid parameters were supplied to Stripe's API"));

            //
        } catch (\Stripe\Error\Authentication $e) {
            // Authentication with Stripe's API failed
            // (maybe you changed API keys recently)
            return redirect()->route('home')->with('alert', __("deposit.Authentication with Stripe's API failed maybe you changed API keys recently"));

        } catch (\Stripe\Error\ApiConnection $e) {
            // Network communication with Stripe failed
            return redirect()->route('home')->with('alert', __("deposit.Network communication with Stripe failed"));
        }catch (Exception $e) {
            // Something else happened, completely unrelated to Stripe
            return redirect()->route('home')->with('alert', __("deposit.Something wrong happened, completely unrelated to Stripe"));
