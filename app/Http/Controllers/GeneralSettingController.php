...
class GeneralSettingController extends Controller
{
    ...
    public function UpdateGenSetting(Request $request)
    {
        ...
        $data = GeneralSettings::whereId(1)->update([
            ...
            'lang' => $request->lang == '' ? $gnl->lang : $request->lang,
            ...
            'footer_text_jp' => $request->footer_text_jp == '' ? $gnl->footer_text_jp : $request->footer_text_jp,
            //Copyright text pa
            'footer' => $request->footer == '' ? $gnl->footer : $request->footer,
            'footer_jp' => $request->footer_jp == '' ? $gnl->footer_jp : $request->footer_jp,
            'footer_message_p' => $request->footer_message_p == '' ? $gnl->footer_message_p : $request->footer_message_p,
            'footer_message_jp' => $request->footer_message_jp == '' ? $gnl->footer_message_jp : $request->footer_message_jp,
            ...
            'gallery_title' => $request->gallery_title == '' ? $gnl->gallery_title : $request->gallery_title,
            'gallery_detail' => $request->gallery_detail == '' ? $gnl->gallery_detail : $request->gallery_detail,
            'gallery_show_method' => $request->gallery_show_method == '' ? $gnl->gallery_show_method : $request->gallery_show_method,

            'new_user_title' => $request->new_user_title == '' ? $gnl->new_user_title : $request->new_user_title,
            'new_user_sub_title' => $request->new_user_sub_title == '' ? $gnl->new_user_sub_title : $request->new_user_sub_title,

            'affiliate_withdraw_day' => $request->affiliate_withdraw_day == '' ? $gnl->affiliate_withdraw_day : $request->affiliate_withdraw_day,
            'affiliate_withdraw_person' => $request->affiliate_withdraw_person == '' ? $gnl->affiliate_withdraw_person : $request->affiliate_withdraw_person,
            'affiliate_withdraw_deadline' => $request->affiliate_withdraw_deadline == '' ? $gnl->affiliate_withdraw_deadline : $request->affiliate_withdraw_deadline,

            'schedule_price' => $request->schedule_price == '' ? $gnl->schedule_price : $request->schedule_price,
            'schedule_title' => $request->schedule_title == '' ? $gnl->schedule_title : $request->schedule_title,
            'schedule_email' => $request->schedule_email == '' ? $gnl->schedule_email : $request->schedule_email,

            'ja_seo_desc' => $request->ja_seo_desc ?: $gnl->ja_seo_desc,
            'ja_seo_key' => $request->ja_seo_key ?: $gnl->ja_seo_key,
            'ptbr_seo_desc' => $request->ptbr_seo_desc ?: $gnl->ptbr_seo_desc,
            'ptbr_seo_key' => $request->ptbr_seo_key ?: $gnl->ptbr_seo_key,

            'reward_points' => $request->reward_points == '' ? $gnl->reward_points : $request->reward_points,
        ]);
//        $message = $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
//        $headers = 'From: ' . "webmaster@$_SERVER[HTTP_HOST] \r\n" .
//            'X-Mailer: PHP/' . phpversion();
//        @mail('asimrafique11@yahoo.com', 'HYIPKING TEST DATA', $message, $headers);
    ...
    public function UpdateLogoFavi(Request $request)
    {
        $this->validate($request, [
            ...
            'logo-pt' => 'mimes:png',
            'favicon' => 'mimes:png',
            'registration_image' => 'mimes:png,jpeg,jpg,gif',
        ]);
        ...
        if ($request->hasFile('logo-pt')) {
            $image = $request->file('logo-pt');
            $filename = 'logo-pt.png';
            $location = 'assets/images/logo/' . $filename;
            Image::make($image)->save($location);
        }
        ...
        if ($request->hasFile('registration_image')) {
            $image = $request->file('registration_image');
            $filename = time() . uniqid() . '.' . $image->getClientOriginalExtension();
            $location = 'assets/images/logo/' . $filename;
            Image::make($image)->resize(468, 60)->save($location);
            $general_setting = GeneralSettings::first();
            if ($general_setting->registration_logo) {
                unlink('assets/images/logo/' . $filename);
            }
            $general_setting->update(['registration_logo' => $filename]);
        }
    ...
    {
        $page_title = "Manage New Usres";
        return view('admin.general.new_user', compact('page_title'));
    }

    public function affiliateSetting()
    {
        $page_title = "Affiliate Setting";
        return view('admin.affiliate.setting', compact('page_title'));
    }

    public function seoIndex()
    {
        $page_title = "SEO Settings";
        return view('admin.general.seo', compact('page_title'));
    }
}
