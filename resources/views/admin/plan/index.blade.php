                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-12">
                                                            <ul class="list-group">
                                                                ...
                                                                <li class="list-group-item">Show in Home: <br> @if($data->show == 1) <span class="badge badge-success">Visible</span> @elseif($data->show == 0) <span class="badge badge-danger">Hidden</span>@endif </li>
                                                                @php $more_fields = json_decode($data->extra) @endphp
                                                                @if($more_fields)
                                                                <li class="list-group-item font-weight-bold">Extra</li>
                                                                    @foreach($more_fields as $field)
                                                                    <li class="list-group-item">{{ $field->key }} : {{ $field->value }}</li>
                                                                    @endforeach
                                                                @endif
                                                                @if($data->image)
                                                                <li class="list-group-item"><img src="{{ asset('assets/images/plan') .'/'. $data->image }}" style="width: 100%"></li>
                                                                @endif
                                                            </ul>
                                                        </div>
                                                    </div>
